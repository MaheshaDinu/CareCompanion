...
public class PatientMedicationController {
    ...
    @GetMapping("/getMedications/{id}")
    public ResponseEntity<ResponseUtil> getMedications(@PathVariable int id, HttpServletRequest request){
        try{
            String authHeader = request.getHeader("Authorization");
            if (authHeader == null || !authHeader.startsWith("Bearer ")) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ResponseUtil(401, "Unauthorized", null));
            ...
            PatientDTO patientDTO = patientService.getByUserName(username);
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseUtil(200,"",medicationScheduleService.getMedicationSchedulesByPatientId(patientDTO.getId())));
        }catch (Exception e){
